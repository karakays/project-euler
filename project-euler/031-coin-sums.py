def find_total_change_ways(coins, coin_index, amount):
    ...
    return find_total_change_ways(coins, coin_index - 1, amount) + \
        find_total_change_ways(coins, coin_index, amount - coins[coin_index])
    ...
    for coin_ind in range(1, len(change_memo)):
        for amount in range(1, len(change_memo[coin_ind])):
            if coins[coin_ind - 1] <= amount:
                change_memo[coin_ind][amount] = \
                    change_memo[coin_ind - 1][amount] + \
                    change_memo[coin_ind][amount - coins[coin_ind - 1]]
            else:
                change_memo[coin_ind][amount] = \
                    change_memo[coin_ind - 1][amount]
